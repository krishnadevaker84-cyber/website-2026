import image1 from "./id1.jpg";
import image2 from "./id2.jpg";
import image3 from './id3.jpg'
import image4 from './id4.jpg'
import image5 from './id5.jpeg'
import image6 from './id6.jpeg'
import image7 from './id7.jpeg'
import image8 from './id8.jpeg'

import { Testimonial } from "@/types/testimonial";

export const testimonialData: Testimonial[] = [
  {
    id: 1,
    name: "Vishal Himmatsinghka",
    designation: "Y21, BT-EE",
    image: image1,
    intern: "SDE Intern @ Atlassian",
    content:"Every error like any failure hides a lesson; persist, just never give up",
    link: "https://medium.com/@ug_anc/the-more-you-know-1-sde-8e04336dc07a"
   } ,
   {
    id: 2,
    name: "Diptansu",
    designation: "Y21, BT-CHE",
    image: image3,
    intern: "RnD Engineer Intern @ Hindustan Unilever",
    content:
      "Don’t worry too much and remember plenty of opportunities will be waiting for you.",
      link:"https://medium.com/@ug_anc/the-more-you-know-2-tm-ccc8f19ab42f"
  },
   {
    id: 3,
    name: "Siddhant Singh",
    designation: "Y21, BT-ME",
    image: image2,
    intern: "Trading Analyst Intern @ Axxela",
    content:
      "Grit and perseverance are your greatest allies. Don’t lose them, and you will find a way.",
      link: "https://medium.com/@ug_anc/the-more-you-know-4-finance-c593f74a9f46"
  },
  {
    id: 4,
    name: "Siddharth Maurya",
    designation: "Y21, BT-CHE",
    image: image4,
    intern: "Strategy and Consulting Associate Intern @ Accenture",
    content:
      "Keep your tenacity alive and consistently offer your best effort, for there’s always a greater reward waiting ahead",
      link: "https://medium.com/@ug_anc/the-more-you-know-3-consulting-503b2694344c"
  },
  {
    id: 5,
    name: "Varun Tokas",
    designation: "Y21, BT-CSE",
    image: image5,
    intern: "Quant Reasearch Intern @ NK Securities",
    content:
      "You can do whatever you like, But you must work hard to get it.",
      link: "https://medium.com/@ug_anc/the-more-you-know-5-quant-57073be30455"
  },
  {
    id: 6,
    name: "Saumya Gupta",
    designation: "Y21, BT-CHE",
    image: image6,
    intern: "Analytics Intern @ American Express",
    content:
      "Embrace your internship preparation journey as a mission, diverse challenges and a ticking clock.",
      link: "https://medium.com/@ug_anc/the-more-you-know-7-analytics-b5c2938559ff"
  },
  {
    id: 7,
    name: "Pranjal Singh",
    designation: "Y21, BT-CHE",
    image: image7,
    intern: "Core Profile Intern @ Dr.Reddy's Laboratory",
    content:
      "Things might not go your way, don't panic or lose hope at that time. Do what is required and everything will fall into place.",
      link: "https://medium.com/@ug_anc/the-more-you-know-6-core-d8701604e530"
  },
  {
    id: 8,
    name: "Rudransh Goel",
    designation: "Y21, BT-EE",
    image: image8,
    intern: "EV-powertrain (Software) Intern @ JLR",
    content:
      "You can do whatever you like, But you must work hard to get it.",
      link: "https://medium.com/@ug_anc/the-pshycology-of-intern-season-3a7bc2352b92"
  },
];
